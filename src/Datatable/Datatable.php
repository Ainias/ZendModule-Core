...
use Ainias\Core\NoDb\ArrayList;
...
class Datatable
{
    ...
    public function __construct($id, $options = array())
    {
        ...
        $this->tableAttributes = array(
            'class' => 'hover stripe compact row-border',
            'width' => '100%',
        );
        ...
        $this->responsiveRenderer = <<<JS
            function(api, rowIdx, columns)
            {
                var data = $.map( columns, function ( col, i ) {
                    return col.hidden ?
                        '<li class = "responsiveDatatableList" data-dt-row="'+col.rowIndex+'" data-dt-column="'+col.columnIndex+'">'+
                            col.title+
                            '<ul><li>'+col.data+'</li></ul>'+
                        '</li>' :
                        '';
                } ).join('');
...
JS;
    ...
    protected function buildJsInitObject()
    {
        ...
        if ($this->isResponsive()) {
            $renderer = $this->responsiveRenderer;
            $responsive =
                <<<JS
                {
                details: {
...
JS;
    ...
    protected function buildFoot()
    {
        if (!$this->isHideTablefoot()) {
            ...
            /** @var Column $column */
            foreach ($this->columns as $column) {
                $tableFood .= $column->buildHeadCell($this->getId(), $i);
                $i++;
            }
            $tableFood .= "</tr></tfoot>";
            return $tableFood;
    ...
    protected function buildHead()
    {
        if (!$this->isHideTablehead()) {
            ...
            /** @var Column $column */
            foreach ($this->columns as $column) {
                $tableHead .= $column->buildHeadCell($this->getId(), $i);
                $i++;
            }
            $tableHead .= "</tr></thead>";
            return $tableHead;
    ...
    public function setOptions(array $options)
    {
        ...
        isset($options["responsiveRenderer"]) && $this->setResponsiveRenderer($options["responsiveRenderer"]);
    }
     ...
     * @return string
     */
    public function getResponsiveRenderer(): string
    {
        return $this->responsiveRenderer;
    }

    /**
     * @param string $responsiveRenderer
     */
    public function setResponsiveRenderer(string $responsiveRenderer)
    {
        $this->responsiveRenderer = $responsiveRenderer;
    }
}
